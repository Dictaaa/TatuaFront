...
export interface PortfolioItem{ id: number; title: string; style: string; duration: string; emoji: string; imageUrl?: string | null; }
...
function styleEmoji(name: string): string {
  const map: Record<string, string> = {
    ...
    'Neotradicional':'🌸','Minimalista':'◇','Lettering':'✍️',
    'Color':'🎨','Cover Up':'🔄','Geométrico':'◆','Acuarela':'💧','Retrato':'👁️',
  };
  return map[name] ?? '✦';
}

function styleDesc(name: string): string {
  const map: Record<string, string> = {
    'Realismo':      'Fotografía en piel. Sombreados que engañan al ojo.',
    'Blackwork':     'Tinta negra pura. Geometría y contraste.',
    'Fine Line':     'Trazos delicados. Elegancia en cada línea.',
    'Old School':    'Iconos clásicos con colores vibrantes.',
    'Neotradicional':'Modernización del estilo clásico.',
    'Minimalista':   'Menos es más. Diseños limpios e impactantes.',
    'Lettering':     'Tipografía como arte permanente.',
    'Color':         'Paletas vibrantes que duran toda la vida.',
    'Cover Up':      'Transformamos tatuajes anteriores.',
    'Geométrico':    'Patrones y formas con precisión matemática.',
    'Acuarela':      'Efecto pintura en tu piel.',
    'Retrato':       'Capturamos rostros con alta fidelidad.',
  };
  return map[name] ?? 'Arte permanente en tu piel.';
}

function formatCOP(value: number | null): string {
  if (!value) return 'Cotización';
...
export class ArtistProfile implements OnInit, OnDestroy {
  ...
  // ── Artist data
  artist     = signal<ArtistData>(MOCK_ARTIST);
  ...
  ngOnInit(): void {
    ...
    // Load artist data from API using the slug from the URL
    const slug = this.route.snapshot.paramMap.get('slug');
    if (slug) {
      this.loadArtist(slug);
    } else {
      this.loadError.set('Artista no encontrado.');
  ...
  private loadArtist(slug: string): void {
    ...
    this.api.get<any>(ENDPOINTS.artists.bySlug(slug)).subscribe({
      next: (data) => {
        // Map API response to ArtistData shape
        const artist: ArtistData = {
          ...
          specialty:  (data.styles ?? []).map((s: any) => s.name),
          tagline:    data.tagline ?? '',
          bio:        data.bio ?? '',
          heroImage:  data.hero_image_url ?? '',
          since:      data.year_started ?? new Date().getFullYear(),
          totalWorks: data.total_tattoos ?? 0,
          rating:     data.rating_avg ?? 0,
          instagram:  data.instagram ?? '',
          whatsapp:   data.whatsapp ?? '',
          facebook:   data.facebook ?? '',
          tiktok:     data.tiktok ?? '',
          styles: (data.styles ?? []).map((s: any) => ({
            name:  s.name,
            emoji: styleEmoji(s.name),
            desc:  styleDesc(s.name),
          })),
          portfolio: (data.portfolio ?? []).map((p: any) => ({
            id:       p.id,
            title:    p.title ?? 'Sin título',
            style:    p.style?.name ?? 'General',
            duration: p.duration_hours ? p.duration_hours + 'h' : '—',
            emoji:    '🎨',
          ...
          services: (data.services ?? []).map((s: any) => ({
            name:  s.name,
            desc:  s.description ?? '',
            from:  s.is_quote_only ? 'Cotización' : formatCOP(s.price_from),
            badge: s.badge ?? 'Desde',
          })),
      ...
      error: (err) => {
        this.isLoading.set(false);
        if (err.status === 404) {
          this.loadError.set('Este artista no existe o su perfil no está disponible.');
        } else {
          this.loadError.set('Error al cargar el perfil. Intenta de nuevo.');
        }
      },
    });
  }
